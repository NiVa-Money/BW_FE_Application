...
import { getChannelsResolvedConversation } from "../../../api/services/mainDashboardServices";
...
const ResolvedChatsChart: React.FC<Props> = ({ payload }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await getChannelsResolvedConversation(payload);
        const metrics = response.data.channelWiseResolvedConversation;
        const transformedData: ResolvedData[] = metrics.map((metric: any, index: number) => ({
          name: `Week ${index + 1}`, // Simplified for chart
          resolved: metric.whatsapp + metric.web,
          total: metric.whatsapp + metric.web + Math.round(Math.random() * 10), // Mocked total (API only provides resolved)
        }));
        setData(transformedData);
      } catch {
        setError("Failed to load resolved chats data");
      } finally {
        setLoading(false);
      }
    };
