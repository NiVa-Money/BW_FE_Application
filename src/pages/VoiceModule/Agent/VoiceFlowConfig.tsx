...
const VoiceFlowConfig = ({ config, updateConfig }: VoiceFlowConfigProps) => {
  ...
  const handleVoiceCloningUpload = async (
    ...
    if (!files || files.length === 0) {
      setCloningStatus("error");
      setCloningMessage("No file selected. Please choose an MP3, WAV, or MPEG file.");
      return;
    }
    ...
    if (!isValidType) {
      setCloningStatus("error");
      setCloningMessage("Invalid file type. Only MP3, WAV, or MPEG files are allowed.");
      event.target.value = ""; // Reset input
      return;
    }
    ...
    try {
      ...
      setCloningMessage(
        error instanceof Error
          ...
          : "Cloning failed. Please try again."
      );
    } finally {
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <span className="text-gray-700 font-medium">
                Upload Voice Samples (MP3/WAV/MPEG)
              </span>
            </label>
            <input
              id="voice-cloning-upload"
              type="file"
              accept=".mp3,.wav,.mpeg"
              className="hidden" // Change to "block" for debugging
              onChange={handleVoiceCloningUpload}
            />
            {cloningMessage && (
              <div
