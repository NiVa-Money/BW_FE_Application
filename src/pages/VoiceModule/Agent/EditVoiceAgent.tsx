...
const EditVoiceAgent = () => {
  const { agentId } = useParams<{ agentId: string }>(); // Get agentId from route
  const [agentConfig, setAgentConfig] = useState({
    basic: {
      name: "",
      language: "english",
    ...
    if (agentId) {
      const fetchAgent = async () => {
        try {
          ...
          setAgentConfig(data); // Assumes API returns data matching agentConfig structure
        } catch (error) {
  ...
  // Handler to update the agent
  const handleUpdateAgent = async () => {
    if (agentId) {
      try {
        const response = await updateVoiceAgentService(agentId, agentConfig);
