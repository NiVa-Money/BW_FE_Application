...
const VoiceChatComponent: React.FC = () => {
  ...
  const [cloningStatus, setCloningStatus] = useState<"idle" | "cloning" | "error">("idle");
  ...
  const updateAgentParameters = async () => {
    try {
      ...
      const chosenModel = language === "en" ? "eleven_flash_v2" : "eleven_flash_v2_5";
      ...
      if (!updateRes.ok) throw new Error(`API Error: ${await updateRes.text()}`);
      console.log("Agent parameters updated successfully.");

      await conversation.endSession();
      await conversation.startSession({ agentId, overrides: { agent: { language } } });
    } catch (error) {
      console.error("Update agent parameters failed:", error);
    }
  };

  const handleVoiceCloningUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCloningMessage('Voice cloned successfully! Available in voice profiles');
      setNewVoiceName('');
      event.target.value = '';
      ...
      setCloningStatus('error');
      setCloningMessage(error instanceof Error ? error.message : 'Cloning failed');
    }
  };
  ...
  return (
    <div>
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="relative shadow-xl rounded-[2.5rem] p-8 max-w-6xl w-full flex flex-col md:flex-row items-center border border-gray-100/80">
          {/* Left Column */}
          <div className="w-full md:w-1/2 space-y-5 p-4">
            ...
            <div className="space-y-1">
              ...
              />
              {cloningMessage && (
                <div className={`mt-2 px-4 py-2.5 rounded-xl backdrop-blur-sm animate-slide-up ${
                  cloningStatus === 'error' 
                    ? 'bg-red-50/90 border border-red-200' 
                    : 'bg-green-50/90 border border-green-200'
                }`}>
                  <p className={`text-sm flex items-center gap-2 ${
                    cloningStatus === 'error' ? 'text-red-700' : 'text-green-700'
                  }`}>
                    {cloningStatus === 'error' ? (
