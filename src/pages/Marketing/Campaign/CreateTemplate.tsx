...
interface CreateTemplateModalProps {
  ...
  onDone: (data: {
    ...
    body: {
      text: string;
      parameters?: { type: "positional"; example: { positional: string[] } };
    };
    footer?: { text: string };
    buttons: TemplateButton[];
  }) => void;
...
const CreateTemplateModal: React.FC<CreateTemplateModalProps> = ({
  ...
  const handleAddButton = () => {
    setButtons((prev) => [
      ...prev,
      { type: "call_to_action", text: "", ctaType: "url", url: "" },
    ]);
  };
  ...
  const handleDone = () => {
    ...
    // ✅ Only create header if it has real content
    let header: { type: string; content: string } | undefined;
    ...
    const formattedButtons = buttons.filter((btn) => btn.text.trim() !== ""); // ✅ Only keep buttons with non-empty text

    onDone({
      name,
      ...(header ? { header } : {}),
      body: bodyPayload,
      ...(footerText.trim() ? { footer: { text: footerText } } : {}), // spreads nothing when empty
      buttons: formattedButtons,
    });
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 z-10 max-h-[80vh] overflow-y-auto">
        ...
        <div className="mt-6">
          ...
          />
          {bodyError && (
            <div className="mt-1 text-xs text-red-600">{bodyError}</div>
          )}
          <button
            onClick={addVariable}
            className="mt-2 px-4 py-2 text-sm font-medium text-white bg-[#65558F] rounded-full hover:bg-purple-950"
