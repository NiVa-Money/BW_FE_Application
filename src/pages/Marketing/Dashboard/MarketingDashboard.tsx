...
const DashboardCard = ({
  ...
  <Card
    ...
    sx={{
      borderRadius: 2,
      backgroundColor: "rgba(101, 85, 143, 0.08)", // 8% opacity background
    }}
  >
    <CardHeader
      action={
...
const MarketingDashboard = () => {
  // State for fetching dashboard insights
  const [loading, setLoading] = useState(false);
  const [newsData, setNewsData] = useState<string[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [currentIndex, setCurrentIndex] = useState(0);
  ...
  // State to toggle between the setup form and the dashboard view
  const [showDashboard, setShowDashboard] = useState(false);

  const navigate = useNavigate();

 const userId = localStorage.getItem("userId");
  // State to control which view to render.
  const [showForm, setShowForm] = useState(false);

  useEffect(() => {
    // If a userId exists (i.e. the user has data), show the form.
    if (userId) {
      setShowForm(true);
    } else {
      setShowForm(false);
  ...
  // Render the MarketingDashboardForm if showForm is true.
  if (showForm) {
  ...
  const formatNewsForCarousel = useCallback(() => {
    return newsData.map((item) => {
      ...
      if (match) {
        // Transform any markdown header (###) into bold markdown
        const transformedContent = match[3].replace(/### (.*)/g, "**$1**");
        return { title: match[2].trim(), content: transformedContent.trim() };
      }
      const transformedItem = item.replace(/### (.*)/g, "**$1**");
  ...
  // If userId exists, we want the user to set up (or update) their marketing insights via the form.
  // Once the form is submitted, we show the dashboard (i.e. the cards).
  if (!showDashboard) {
    return <MarketingDashboardForm />;
  }
