...
import { formatText } from "../../hooks/functions";

const TestBot: React.FC<any> = ({ botId, onClose }) => {
  ...
  const botReduxName = useSelector(
    (state: RootState) =>
      state.bot?.lists?.data?.find((item: any) => item._id === botParamId)
        ?.botName
  );
      ...
      <div className=" flex flex-col h-[75vh] w-[450px] bg-[#1e1b20] ">
        <div className="flex justify-between h-[80px] bg-[#000000] p-[15px]">
          <div>
            ...
            <span className="text-white text-[0.8rem]">Online</span>
          </div>
          <button
            className="self-end bg-[#3f2181] text-white p-1 w-[100px] rounded-[100px] mb-2 mt-4 mr-4"
            onClick={closeTestBotHandler}
          >
            End Chat
          </button>
        </div>
        <div className="h-[calc(100% - 120px)] h-[95%] w-[95%] m-auto overflow-y-scroll">
          {sessionMessages.map((msg, index) => (
            <div key={index} className="flex flex-col space-y-2">
              {/* Answer on the left */}
              <div className="self-end bg-purple-600 text-white mb-4 px-2 py-2 rounded-lg max-w-xs">
                <span className="flex gap-[5px] mb-2 justify-between">
                  {msg.question}
                  <AccountCircleIcon />
                </span>
              </div>
              {/* Question on the right */}
              <div className="self-start bg-gray-800 text-white mb-4 mt-4  px-2 py-2 rounded-lg max-w-xs">
        ...
        <div className="flex  justify-between items-center  m-[12px]">
          ...
          <Button onClick={senMessageHandler}>
            <SendIcon className="mr-[12px] text-white" />
          </Button>
        </div>
      </div>
    </div>
