...
//     // Ensure a bot is selected before searching
//     if (!botIdVal) {
...
//         notifyError(
//           "Order name must be in the format 'NK/12345' (any number of digits allowed)"
//         );
//         return;
//       }
//       data.orderId = searchValue.trim();
//     } else {
...
//         return;
//       }
//       data.userPhoneId = "+91" + searchValue; // Hardcoded country code
//     }

//     // Dispatch the API call
//     const response: any = await dispatch(getAllSession(data));

//     // If successful, store the sessions in local state
//     if (response?.payload?.success) {
//       // This depends on how your API returns data;
//       // assuming sessions are in response.payload.data.sessions
//       setSearchedSessions(response.payload.data?.sessions || []);
//     }
//   };
...
//         >
//           <option value="order">Search by OrderId</option>
...
const AllChats = () => {
  ...
  const handleSearch = async () => {
    ...
    if (searchType === "order") {
      ...
      if (!orderRegex.test(searchValue.trim())) {
        notifyError(
          "Order ID must be in the format 'NK/12345' (any number of digits allowed)"
        );
      ...
      // Validate Phone Number has exactly 10 digits
      if (searchValue.trim().length !== 10 || !/^\d+$/.test(searchValue.trim())) {
        notifyError("Please enter a valid 10-digit phone number");
        setIsSearching(false);
        return;
      }
    ...
    if (searchType === "order") {
      ...
      data.userPhoneId = "+91" + searchValue.trim();
    }
    ...
    try {
      ...
      // If successful, store the search state
      if (response?.payload?.success) {
        ...
        notifyError("Search failed: " + (response?.payload?.message ));
      }
    } catch (error) {
  ...
  const getChatHistory = async ({
    ...
    // If search is active, include search parameters
      ...
      {isSearchActive && (
        <div className="mb-2 px-4 py-2 bg-blue-100 text-blue-800 rounded flex justify-between items-center">
          <span>
            ...
            <strong>{searchType === "phone" ? "+91" : ""}{searchValue}</strong>
          </span>
