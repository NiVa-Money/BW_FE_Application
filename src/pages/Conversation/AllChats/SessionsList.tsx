...
const SessionsList: React.FC<SessionsListProps> = ({
  ...
  useEffect(() => {
    if (sessionsDataRedux?.success) {
      ...
      // Filter by AI or Human only if one is selected exclusively.
      if (aiLevel !== humanLevel) {
        filteredSessions = filteredSessions.filter(
          (session: any) =>
            (aiLevel && session.handledBy === "AI") ||
  ...
  }, [sessionsDataRedux, aiLevel, humanLevel, searchType, searchValue, isSearchActive]);
  ...
  return (
    <div className="w-64 pl-0 bg-white p-4 border-r overflow-y-scroll">
      ...
      {/* Render pagination if there are at least 20 sessions */}
      {sessionsData.length >= 20 && setPage && page !== undefined && (
        <div className="flex justify-between mt-4">
          <button
            disabled={page === 1}
