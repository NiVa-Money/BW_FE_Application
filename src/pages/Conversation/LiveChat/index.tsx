...
const LiveChat: React.FC = (): React.ReactElement => {
  ...
  // ---------- State ----------
  const [isAgentAssistOpen, setIsAgentAssistOpen] = useState(true);
  const [messages, setMessages] = useState<any[]>([{ sender: "agent", text: "What's Your Mood?", emoji: "😊" }]);
  const [selectedSessionId, setSelectedSessionId] = useState<string>("");
  const [newMessage, setNewMessage] = useState<string>("");
  const [isChatEnabled, setIsChatEnabled] = useState(false);
  const [showConfirmationModal, setShowConfirmationModal] = useState(false);
              ...
              <div className="bg-white rounded-lg shadow-md p-4">
                ...
                <div className="overflow-y-auto max-h-[60vh]">
                  {messages?.map((msg: any, index: number) => (
                    <div key={index} className="flex flex-col space-y-2">
                      {msg?.text && (
                        <div className={`flex ${msg.sender === "agent" ? "justify-start" : "justify-end"} mb-4`}>
                          {msg.sender === "agent" && (
                            <div className="w-8 h-8 rounded-full bg-gray-200 mr-2 flex items-center justify-center">
                              {msg.emoji || <SmartToy className="text-gray-600 w-6 h-6" />}
                            </div>
                          )}
                          <div className={`p-3 rounded-lg max-w-[70%] ${msg.sender === "agent" ? "bg-gray-100 text-black" : "bg-[#2E2F5F] text-white"}`}>
                            {msg.text}
                          </div>
                          {msg.sender !== "agent" && (
                            <div className="w-8 h-8 rounded-full bg-[#2E2F5F] ml-2 flex items-center justify-center">
                              <Person className="text-white w-6 h-6" />
                            </div>
                          )}
                        </div>
                    ...
                    <div>
                      <form onSubmit={sendMessage} className="mt-2 flex items-center gap-2">
                        <input
                          type="text"
                          placeholder="Type your message..."
                          className="flex-1 p-3 rounded-l-lg bg-white/10 text-white outline-none"
                          value={newMessage}
                          onChange={(e) => setNewMessage(e.target.value)}
                          onFocus={() => setIsAgentTyping(true)}
                          onBlur={() => setIsAgentTyping(false)}
                          disabled={!isAgentConnected}
                        />
                        <button type="submit" className="bg-white/10 p-3 rounded-r-lg text-white">
                          <Send />
                        </button>
                      </form>
                      <div className="flex justify-center items-center gap-4 mt-4">
                        <div className="relative">
                          <button
                            ...
                            className="w-10 h-10 rounded-full bg-purple-100 text-[#65558F] flex items-center justify-center hover:bg-purple-200"
                          >
                            😊
                          </button>
                          {showEmojiPicker && (
                        ...
                        <div className="relative">
                          <button
                            onClick={() => {/* Toggle suggested responses visibility */}}
                            className="w-10 h-10 rounded-full bg-purple-100 text-[#65558F] flex items-center justify-center hover:bg-purple-200"
                          >
                            💡
                          </button>
                          {suggestedResponses.length > 0 && (
                            <div className="absolute bottom-full mb-2 bg-white border rounded shadow-lg p-2">
                              {suggestedResponses.map((response, index) => (
                                <button
                                  key={index}
                                  className="block w-full text-left px-2 py-1 text-sm text-[#65558F] hover:bg-gray-100"
                                  onClick={() => sendMessageQuick(response.message)}
                                >
                                  {response.message}
                                </button>
