...
import { getAllBotsService } from "../../../api/services/agentBuilderServices";
...
const LiveChat: React.FC = (): React.ReactElement => {
  ...
  const [botLists, setBotLists] = useState<{ value: string; name: string }[]>(
    []
  );
      ...
      try {
        ...
        if (response.success) {
          ...
          if (formattedBots.length > 0) {
            setBotId(formattedBots[0].value);
          }
        } else {
  ...
  return (
    <div className="h-screen flex flex-col relative">
      ...
      {/* Optional: Render a loading screen while fetching bots */}
      {isLoading ? (
      ...
      {/* Conditionally render the Agent Assist button only when not loading */}
      {!isLoading && (
        <button
