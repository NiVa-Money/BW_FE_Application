...
const EditBot: React.FC = () => {
  ...
  const fetchBotData = async () => {
    ...
    try {
      const data = await getBotProfileByIDService(cleanedBotId);
  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    ...
    try {
      ...
      console.log("Edit Bot Profile Response:", response);

      // ✅ Use response.success to control the modal
      if (response?.success) {
        setIsModalOpen(true);
      } else {
        console.error("Edit failed:", response?.message || "Unknown error");
      }
    } catch (error: any) {
      console.error(
        "Submission failed:",
  ...
  return (
    <div className="p-6 ml-2 max-w-7xl mx-auto">
      ...
      <div className="flex">
        <div className="flex-1 max-w-4xl pr-6">
          <Formik
          ...
          >
            {(formik) => (
              <div className="flex">
                {/* Form Section */}
                <Form className="flex-2 space-y-6">
                ...
                >
                  <div
                    className="h-[500px] sticky top-20 mb-10"
                    style={{
                      overflow: "visible", // Ensure no clipping
                    }}
                  >
